...
public class DefaultPriceService implements PriceService {
    ...
    Flux<MessageDTO<Float>> averagePrice(Flux<Long> requestedInterval,
        ...
        return requestedInterval
            ...
            .switchMap(t ->
                priceData.window(Duration.ofSeconds(t))
                    .flatMap(messageDTOFlux ->
                        messageDTOFlux.groupBy(MessageDTO::getCurrency)
                            .flatMap(groupedFlux -> groupedFlux
                                .reduce(Sum.empty(), (acc, message) -> acc.add(message.getData()))
                                .map(Sum::avg)
                                .map(avgVal ->
                                        MessageDTO.avg(avgVal, groupedFlux.key(), "")))
                        ));
