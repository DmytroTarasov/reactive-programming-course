...
public class Task {
	...
	public static Flux<IceCreamBall> fillIceCreamWaffleBowl(Flux<IceCreamType> clientPreferences,
		...
		return clientPreferences.switchMap(iceCreamType -> {
			if (iceCreamType == IceCreamType.CHOCOLATE) {
				return chocolateIceCreamStream;
			} else if (iceCreamType == IceCreamType.VANILLA){
				return vanillaIceCreamStream;
			} else {
				return Flux.error(new IllegalArgumentException("An unknown ice cream type was provided"));
			}
		});
	}
